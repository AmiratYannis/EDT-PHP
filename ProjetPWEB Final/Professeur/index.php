<?php 
	session_start ();
	//$_SESSION['profil'];
	if ((count($_GET)!=0) && !(isset($_GET['controle']) && isset ($_GET['action'])))require ('./vue/erreur404.tpl'); //cas d'un appel à index.php avec des paramètres incorrects
	
	else {
		if ((! isset($_SESSION['profil'])) || count($_GET)==0)	{
			$controle = "professeur";   //cas d'une personne non authentifiée
			$action = "identProf";		//ou d'un appel à index.php sans paramètre
		}
		else {
